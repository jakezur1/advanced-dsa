...
public:
    SegmentTree(vector<int> data) {
        dataSize = data.size();
        treeSize = isPower2(dataSize) ? 2 * dataSize - 1: 2 * closestPower(dataSize) - 1;
        tree = vector<int>(treeSize, INT_MAX);
        build(data, 0, 0, dataSize - 1);
    }

    int queryRange(int qL, int qR) { return _queryRange(qL, qR, 0, 0, dataSize - 1); }

    void update(int idx, int val) { return _update(idx, val, 0, 0, dataSize - 1); }
...
private:
    ...
    void build(vector<int>& data, int idx, int l, int r) { 
        if (l == r) {
            tree[idx] = data[l];
            return;
        }
    ...
    int _queryRange(int qL, int qR, int idx,  int l, int r) {
        ...
        return combine(_queryRange(qL, m, leftChildIdx, l, m),  _queryRange(m + 1, qR, rightChildIdx, m + 1, r));
    }
