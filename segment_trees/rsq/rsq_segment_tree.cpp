...
RSQSegmentTree::RSQSegmentTree (vector<int> data) {
    dataSize = data.size();
...
int RSQSegmentTree::closestPower(int n) {
    return pow(2, ceil(log2(n)));
}

bool RSQSegmentTree::isPower2(int n) {
    return n > 0 && (n & (n - 1)) == 0;
}

void RSQSegmentTree::build(vector<int>& data, int l, int r, int idx) {
    if (l == r) {
        tree[idx] = data[l];
        return;
    }
...
int RSQSegmentTree::_querySumRange(int qL, int qR, int idx,  int l, int r) {
    ...
    return  _querySumRange(qL, m, leftChildIdx, l, m) + _querySumRange(m + 1, qR, rightChildIdx, m + 1, r);
}